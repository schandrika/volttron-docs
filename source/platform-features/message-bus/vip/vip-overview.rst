...
VIP makes no assumptions about the security mechanisms used. It works equally well over encrypted or unencrypted
channels. Any connection-level authentication and encryption is handled by ZAP. Message-level authentication can be
implemented in the protocols and services using VIP or by utilizing message properties set in ZAP replies.
...
The errors above are reported via the *error* subsystem. Other errors MAY be reported via the *error* subsystem,
but subsystems SHOULD provide mechanisms for reporting subsystem-specific errors whenever possible.
...
Reference VIP router: https://github.com/eclipse-volttron/volttron-core/blob/main/src/volttron/server/router/base_router.py

Reference VIP peer: https://github.com/eclipse-volttron/volttron-core/blob/main/src/volttron/utils/socket.py
